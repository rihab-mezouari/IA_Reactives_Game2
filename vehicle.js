  ...
  class Vehicle {
    ...
    suivre(position, distancevoulu = 60) {
      ...
      if (Vehicle.debug) {
        stroke(255, 0, 255);
        noFill();
        circle(target.x, target.y, this.wanderRadius * 2);
      }
    ...
    */
    ...
    show() {
      ...
      // Affichage du cercle de debug
      if (Vehicle.debug) {
        push();
        noFill();
        stroke("white"); // Couleur rouge transparente
        strokeWeight(2);
        circle(this.pos.x, this.pos.y, 100); // Cercle autour du véhicule
        pop();
      }
    }
